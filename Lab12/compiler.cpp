...
struct ThreeAddressCode {
    ...
    ThreeAddressCode(string res, string op1, string op2, string oper)
        : result(res), operand1(op1), operand2(op2), op(oper) {}
...
class Parser
{
    ...
    private:
        ...
        void parseAssignment() {
            ...
            expect(T_ASSIGN);
            parseExpression();
            // Generate Three Address Code for assignment
            tac.push_back(ThreeAddressCode(idToken.value, "t" + to_string(tac.size()), "", "="));
            expect(T_SEMICOLON);
        }
        ...
        void parseExpression() {
            Token operand1 = tokens[pos];
            string tempVar = "t" + to_string(tac.size() + 1); 
            parseTerm();
            while (tokens[pos].type == T_PLUS || tokens[pos].type == T_MINUS) {
                Token op = tokens[pos];
                pos++;
                Token operand2 = tokens[pos];
                parseTerm();

                // Generate Three Address Code for the operation
                tac.push_back(ThreeAddressCode(tempVar, operand1.value, operand2.value, op.value));

                // Update the operand to the new temporary variable
                operand1.value = tempVar;
            }
